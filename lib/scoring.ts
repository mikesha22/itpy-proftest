...
export function ogeRecommendation(scores: Scores) {
  if (scores.ogeIndex >= 70 && scores.interest >= 55 && scores.study >= 50) {
    return "Текущий результат показывает хорошую основу для подготовки к ОГЭ по информатике. Полезно познакомиться с форматом экзамена и проверить уровень на нескольких типовых заданиях.";
  }
  if (scores.ogeIndex >= 55) {
    return "Есть основа для подготовки к ОГЭ по информатике. Несколько занятий и знакомство с заданиями экзамена помогут точнее оценить дальнейший план.";
  }
  if (scores.ogeIndex >= 40) {
    return "Для подготовки к ОГЭ потребуется последовательно укрепить базовые темы и регулярно решать задания экзамена.";
  }
  return "Начать подготовку к ОГЭ лучше с повторения базовых тем и спокойного плана регулярных занятий.";
}
...
export function egeRecommendation(scores: Scores, grade?: string) {
  if (grade && isEgeOnlyGrade(grade)) {
    if (scores.egeIndex >= 70 && scores.interest >= 60 && scores.logic >= 55) {
      return "Текущий результат показывает хорошую основу для подготовки к ЕГЭ по информатике. Следующий шаг — познакомиться с форматом экзамена и углубить навыки программирования.";
    }
    if (scores.egeIndex >= 55) {
      return "Есть основа для подготовки к ЕГЭ по информатике. Следующий шаг — попробовать Python и несколько заданий экзамена.";
    }
    if (scores.egeIndex >= 40) {
      return "Для подготовки к ЕГЭ потребуется системная работа. Начать лучше с диагностики и базовых заданий экзамена.";
    }
    return "Начать подготовку к ЕГЭ лучше с повторения основ и последовательной работы по темам экзамена.";
  }

  if (scores.egeIndex >= 70 && scores.interest >= 60 && scores.logic >= 55) {
    return "Текущий результат показывает хорошую основу для дальнейшей подготовки к ЕГЭ и более глубокого знакомства с программированием.";
  }
  if (scores.egeIndex >= 55) {
    return "Есть основа для дальнейшей подготовки к ЕГЭ. Следующий шаг — попробовать Python и более продолжительный учебный формат.";
  }
  if (scores.egeIndex >= 40) {
    return "Для подготовки к ЕГЭ потребуется системная работа. Начать лучше с Python и базовых алгоритмов.";
  }
  return "Подготовку к ЕГЭ лучше начать с укрепления базовых навыков и постепенно переходить к заданиям экзамена.";
}
